...
class Dungeon {
  ...
  constructor(id, user, dungeonCode) {
    ...
    this.player = new Player(user);     // 플레이어 정보
    this.mapCode;                       // 맵 구별 값 (5000 ~ 5006)
    this.dungeonCode = dungeonCode;     // 던전 구분 (1 ~ 4)
    this.stages = [];                   // 스테이지 목록
    this.proceed = 0;                   // 던전 진행도 (인덱스 값: 0 ~ lastStage-1)
    this.lastStage = [4, 5, 6, 7];      // 스테이지 별 길이

    this.loadedAssets = getGameAssets();      // (서버 시작시 로드된) 데이터 가져오기
  }
  ...
  initStage() {
    ...
    for (let j = 0; j < maxNumber; j++) {      // 스테이지당 1~3 마리의 몬스터 생성
      ...
      if (this.proceed + 1 == this.lastStage) {
        ...
        if (this.dungeonCode <= 2) { monsterModel = 2000 + monsterIdx; }      // 1,2 던전은 초급보스 이미지 | (2016~2021)
        else if (this.dungeonCode >= 3) { monsterModel = 2000 + monsterIdx; }     // 3,4 던전은 고급보스 이미지 | (2022~2027)
      }
      else {
        ...
        else if (this.dungeonCode >= 3) { monsterModel = 2001 + monsterIdx; }     // 3,4 던전은 고급 일반몹 이미지 | (2008~2015)
      }

      let monster_Name = '';
      if (this.proceed + 1 == this.lastStage) { monster_Name = `[BOSS] ${monsterDatas[monsterIdx].monsterName}` }
    ...
    else { msg = `${this.user.nickname}, 던전${this.dungeonCode} ${this.proceed + 1}스테이지 도달!`; }
    ...
    // battleLog 생성
    if(this.proceed + 1 == this.lastStage) { msg = `적 보스 ${monsters[0].getName()} 출현!`; }
    else { msg = `적 ${maxNumber} 개체 출현!`; }      // screenText도 Button도 같은 이름의 요소 'msg'를 가지고 있음
    ...
    for (let l = 0; l < 2; l++) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        if (l == 0) {     // 일반공격 버튼생성
          ...
          else { newBtn = new Btn(`[공격] ${monsters[k].getName()}`, true); }
    ...
    if (this.proceed + 1 == this.lastStage) {      // 마지막 방 (보스방) 일 경우 - 현재 구현중 임
